import { useEffect, useState } from 'react';
import { Formik, Form, type FormikHelpers } from 'formik';
import { action, createContextStore } from 'easy-peasy';
import type { Action } from 'easy-peasy';
import tw from 'twin.macro';
import { bool, object, string } from 'yup';

import type { UpdateUserValues } from '@/api/routes/admin/users';
import { getRole } from '@/api/routes/admin/roles';
import AdminBox from '@/elements/AdminBox';
import CopyOnClick from '@/elements/CopyOnClick';
import FormikSwitch from '@/elements/FormikSwitch';
import Input from '@/elements/Input';
import Label from '@/elements/Label';
import SpinnerOverlay from '@/elements/SpinnerOverlay';
import { Button } from '@/elements/button';
import Field, { FieldRow } from '@/elements/Field';
import { Alert } from '@/elements/alert';
import { UserRole, type User } from '@definitions/admin';
import { faIdBadge, faToggleOn } from '@fortawesome/free-solid-svg-icons';
import { useStoreState } from '@/state/hooks';
import RoleSelect from './RoleSelect';

// Optional context (commented out if unused)
/*
interface Ctx {
    user: User | undefined;
    setUser: Action<Ctx, User | undefined>;
}

export const Context = createContextStore<Ctx>({
    user: undefined,
    setUser: action((state, payload) => {
        state.user = payload;
    }),
});
*/

export interface Params {
    title: string;
    initialValues?: UpdateUserValues;
    children?: React.ReactNode;
    uuid?: string;
    admin_role_id?: number | null;

    // Better type alignment with Formik
    onSubmit: (values: UpdateUserValues, helpers: FormikHelpers<UpdateUserValues>) => void;
}

export default function UserForm({
    title,
    initialValues,
    children,
    onSubmit,
    uuid,
    admin_role_id,
}: Params) {
    const { colors } = useStoreState((state) => state.theme.data!);
    const currentUser = useStoreState((state) => state.user.data);

    const [currentRole, setCurrentRole] = useState<UserRole | undefined>();
    const [loadingRole, setLoadingRole] = useState(false);

    // Avoid prop mutation – use a local default
    const defaultValues: UpdateUserValues = {
        externalId: '',
        username: '',
        email: '',
        password: '',
        admin_role_id: null,
        state: '',
        rootAdmin: false,
    };
    const formValues = initialValues ?? defaultValues;

    // Fetch role if admin_role_id is provided
    useEffect(() => {
        if (admin_role_id) {
            setLoadingRole(true);
            getRole(Number(admin_role_id))
                .then(setCurrentRole)
                .catch((error) => console.error('Failed to fetch role:', error))
                .finally(() => setLoadingRole(false));
        } else {
            setCurrentRole(undefined);
        }
    }, [admin_role_id]);

    // Validation schema moved out for clarity
    const validationSchema = object().shape({
        username: string()
            .min(1, 'Username is required')
            .max(32, 'Username too long')
            .required('Username is required'),
        email: string().email('Invalid email format').required('Email is required'),
        rootAdmin: bool().required(),
    });

    const handleSubmit = (values: UpdateUserValues, helpers: FormikHelpers<UpdateUserValues>) => {
        onSubmit(values, helpers);
    };

    return (
        <Formik initialValues={formValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
            {({ isSubmitting, isValid }) => (
                <Form>
                    {/* ─── User Info Section ─────────────────────────────── */}
                    <AdminBox title={title} css={tw`relative`} icon={faIdBadge}>
                        <SpinnerOverlay visible={isSubmitting} />
                        <FieldRow>
                            {uuid && (
                                <div>
                                    <Label>UUID</Label>
                                    <CopyOnClick text={uuid}>
                                        <Input type="text" value={uuid} readOnly />
                                    </CopyOnClick>
                                </div>
                            )}
                            {uuid && (
                                <Field
                                    id="externalId"
                                    name="externalId"
                                    label="External ID"
                                    type="text"
                                    description="Used by external integrations; do not modify unless necessary."
                                />
                            )}
                            <Field
                                id="username"
                                name="username"
                                label="Username"
                                type="text"
                                description="The user's username."
                            />
                            <Field
                                id="email"
                                name="email"
                                label="Email Address"
                                type="email"
                                description="The user's email address."
                            />
                            <Field
                                id="password"
                                name="password"
                                label="Password"
                                type="password"
                                placeholder="••••••••"
                                autoComplete="new-password"
                                description="Leave empty to email the user a link to set their password."
                            />
                        </FieldRow>
                    </AdminBox>

                    {/* ─── Permission Control Section ─────────────────────── */}
                    <AdminBox title="Permission Control" css={tw`relative mt-6`} icon={faToggleOn}>
                        <SpinnerOverlay visible={isSubmitting || loadingRole} />
                        <div className="grid lg:grid-cols-2 gap-4">
                            {currentUser?.rootAdmin && (
                                <div css={tw`w-full flex flex-row mb-6`}>
                                    <div
                                        css={tw`w-full border border-neutral-900 shadow-inner p-4 rounded`}
                                        style={{ backgroundColor: colors.headers }}
                                    >
                                        <FormikSwitch
                                            name="rootAdmin"
                                            label="Root Admin"
                                            description="Should this user be a root administrator?"
                                        />
                                        <Alert type="warning" className="mt-2" small>
                                            Enabling Root Admin gives the user full access. It is recommended to keep
                                            this off and assign an admin role instead.
                                        </Alert>
                                    </div>
                                </div>
                            )}

                            <div>
                                <RoleSelect selected={currentRole} />
                                <p className="mt-1 text-xs">
                                    Assign an administrator role to restrict permissions if needed.
                                </p>
                            </div>
                        </div>
                    </AdminBox>

                    {/* ─── Submit Row ────────────────────────────────────── */}
                    <div css={tw`w-full flex flex-row items-center mt-6`}>
                        {children}
                        <div css={tw`flex ml-auto`}>
                            <Button type="submit" disabled={isSubmitting || !isValid}>
                                Save Changes
                            </Button>
                        </div>
                    </div>
                </Form>
            )}
        </Formik>
    );
}
